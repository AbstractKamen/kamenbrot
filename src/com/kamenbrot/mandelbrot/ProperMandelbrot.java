...
public class ProperMandelbrot extends JPanel {
    ...
    public ProperMandelbrot() {
        ...
        this.mandelState = new MandelDoubleState();
    ...
    private int mandelbrotAt(int x, int y) {
        ...
        return mandelbrot(new Complex(real, imaginary));
    }

    private int juliaAt(int x, int y) {
        final double real = Mapping.mapComplex(x, mandelState.getMandelWidth(), mandelState.getMinX(), mandelState.getMaxX());
        final double imaginary = Mapping.mapComplex(y, mandelState.getMandelHeight(), mandelState.getMinY(), mandelState.getMaxY());
        return julia(new Complex(real, imaginary));
    }

    private int mandelbrot(Complex c) {
        Complex z = new Complex(0, 0);
        int n = 0;
        while (z.abs() <= 2 && n < MAX_ITERATIONS) {
            z = z.times(z).plus(c);
            n++;
        }
        return n;
